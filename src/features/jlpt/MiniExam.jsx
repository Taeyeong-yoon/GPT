...
const MINI_TIME = 15 * 60;

export default function JlptMiniExam() {
  const navigate  = useNavigate();
  const location  = useLocation();
  const { user }  = useAuth();
  const level     = location.state?.level || 'N5';
  const isPro     = location.state?.isPro ?? false;
  const { examSet, loading, error } = useJlptQuestions(level, true);
  ...
  useEffect(() => {
    if (examSet[current]?.section === 'listening') { setPlayCount(0); setTtsError(''); }
  }, [current, examSet]);

  const handlePlay = useCallback(async (text, subType) => {
    if (ttsPlaying || !text) return;
    setTtsPlaying(true); setTtsError('');
    try { await speakListening(text, subType); setPlayCount(c => c + 1); }
    catch { setTtsError('재생 실패 — 다시 눌러주세요'); }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      const ref = await addDoc(collection(db, 'users', user.uid, 'results'),
        { type: 'jlpt_mini', level, ...result, createdAt: serverTimestamp() });
      await incrementMiniUsage(user.uid, isPro, 'jlpt', level);
      navigate(`/jlpt/mini/result/${ref.id}`, { state: { result, level } });
    } catch {
      await incrementMiniUsage(user.uid, isPro, 'jlpt', level);
      navigate('/jlpt/mini/result/local', { state: { result, level } });
    }
  }, [examSet, answers, level, user, isPro, submitting]);
  ...
  if (error) return (
    <div className="nm-app" style={{alignItems:'center',justifyContent:'center',gap:16}}>
      ...
      <button className="btn btn--primary" onClick={() => navigate('/jlpt/mini')}>돌아가기</button>
    </div>
  );
  ...
  return (
    <div>
      <div className="exam-header">
        <span className="exam-header__section">미니 · {SECTION_LABELS[sec]}</span>
        <div className="exam-header__progress">
          <div className="progress"><div className="progress__fill" style={{width:`${pct}%`}}/></div>
        </div>
        <span className={`exam-header__time ${timeLeft < 180 ? 'is-warning' : ''}`}>{mm}:{ss}</span>
      </div>

      <div className="screen">
        <div className="question-card">
          <div className="question-card__top">
            <span className={`chip chip--${level.toLowerCase()}`}>{level} 미니</span>
      ...
      <div className="cta-bar">
        <button className="btn btn--primary btn--block" onClick={handleNext}
          ...
          {current < examSet.length - 1 ? '다음' : '결과 보기'}
        </button>
      </div>
    </div>
  );
