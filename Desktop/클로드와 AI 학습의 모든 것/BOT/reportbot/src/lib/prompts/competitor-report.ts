...
const SYSTEM = `You are a senior Korean market intelligence analyst. Your job is to analyze ANY CSV data and produce a professional Korean competitor analysis / business intelligence report.

IMPORTANT: The CSV columns will vary for every customer. Do NOT assume fixed column names like "경쟁사", "점유율". Read the actual headers and adapt your analysis to whatever data is provided. Use the actual column names from the CSV as-is.
...
export const competitorReportPrompt = {
  ...
  buildUserPrompt: ({ companyName, period, additionalNotes, data }: PromptParams) => {
    ...
    return JSON.stringify(
      {
        task: "Generate a Korean competitor analysis / business intelligence report based on the CSV data below. Adapt your analysis to whatever columns exist in the data. Return raw JSON only.",
        companyName,
        period,
