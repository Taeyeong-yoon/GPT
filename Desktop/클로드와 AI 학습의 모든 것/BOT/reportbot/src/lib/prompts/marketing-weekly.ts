...
const SYSTEM = `You are a senior Korean marketing analyst. Your job is to analyze ANY CSV data and produce a professional Korean weekly marketing performance report.

IMPORTANT: The CSV columns will vary for every customer. Do NOT assume fixed column names like "채널", "세션수", "ROAS". Read the actual headers and adapt your analysis to whatever data is provided. Use the actual column names from the CSV as-is.
...
export const marketingWeeklyPrompt = {
  ...
  buildUserPrompt: ({ companyName, period, additionalNotes, data }: PromptParams) => {
    ...
    return JSON.stringify(
      {
        task: "Generate a Korean weekly marketing performance report based on the CSV data below. Adapt your analysis to whatever columns exist in the data. Return raw JSON only.",
        companyName,
        period,
