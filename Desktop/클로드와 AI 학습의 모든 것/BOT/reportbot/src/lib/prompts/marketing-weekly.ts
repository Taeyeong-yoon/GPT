...
const SYSTEM = `You are a senior Korean marketing analyst. Return ONLY valid JSON — no markdown, no explanation.
...
export const marketingWeeklyPrompt = {
  ...
  buildUserPrompt: ({ companyName, period, additionalNotes, data }: PromptParams) =>
    JSON.stringify(
      {
        task: "Generate a Korean weekly marketing performance report. Return JSON only.",
        companyName,
        period,
